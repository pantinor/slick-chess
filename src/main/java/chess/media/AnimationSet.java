...
public class AnimationSet {
    ...
    public AnimationSet(String filename, boolean swapColor) throws Exception {
        ...
        for (int i = 0; i < max; i++) {
            east.addFrame(sheet.getSprite(0, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            north.addFrame(sheet.getSprite(1, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            northeast.addFrame(sheet.getSprite(2, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            northwest.addFrame(sheet.getSprite(3, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            south.addFrame(sheet.getSprite(4, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            southeast.addFrame(sheet.getSprite(5, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            southwest.addFrame(sheet.getSprite(6, i), DURATION);
        }
        for (int i = 0; i < max; i++) {
            west.addFrame(sheet.getSprite(7, i), DURATION);
        }
