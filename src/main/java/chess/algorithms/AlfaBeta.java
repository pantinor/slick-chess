...
/**
 * AlfaBeta, a subclass of MoveAlgorithm, is an alfabeta search algorithm. This
 * technique is called "alfa-beta pruning". It is a refined minimax strategy
 * that identifies unfavorable branches in the move tree and removes them early,
...
public class AlfaBeta extends MoveAlgorithm implements Serializable {
	...
	/**
	 * The search algorithm, using methods found in the super class
	 * MoveAlgorithm, produces the best move result in mm
	 * 
	 * @param white
	 *            - White player ?
	 * @param alpha
	 *            - The best score
	 * @param beta
	 *            - The worst-case scenario for the opponent.
	 * @param depth
	...
	private int alfaBeta(boolean white, int alpha, int beta, int depth) {
		...
		if (v != null) {
			int siz = v.size();
			if (depth == dd)
				super.board.progress.setMax(v.size());
			v = randomize(board.pieces, v);
			while (v.size() > 0 && best < beta) {
				...
				m.undo(board);
				if (depth == dd) {
					super.board.think = (mm == null ? "" : mm + " - ") + v.toString();
					super.board.progress.setValue(siz - v.size());
					System.out.println(super.board.think);
				}
			}
		}
		return best;
