...
public class MoveAlgorithm implements Constants, Serializable {
     ...
     */
    protected Move getReply(boolean white) {
        return mm;
    }

    private Vector getPawnMoves(Coord c) {
        Vector v = new Vector();
        if (board.pieces[c.x][c.y].white) {
            if (board.pieces[c.x][c.y + 1] == null) { // empty block
            ...
            if (board.pieces[c.x][c.y - 1] == null) {
                v.add(new Move(c.x, c.y, c.x, c.y - 1));
                if (c.y == 6) {
                    v.add(new Move(c.x, c.y, c.x, c.y - 2));
                }
    ...
    private Vector getBishopAttacks(Coord c) {
        ...
        while (c.x + i <= 7 && c.y + i <= 7) {
            ...
            if (board.pieces[c.x + i][c.y + i] != null) {
                break;
            }
            i++;
        }
    ...
    private Vector getRookAttacks(Coord c) {
        ...
        while (c.y + i <= 7) {
            ...
            if (board.pieces[c.x][c.y + i] != null) {
                break;
            }
            i++;
        }
        ...
        while (c.x - i >= 0) {
            ...
            if (board.pieces[c.x - i][c.y] != null) {
                break;
            }
            i++;
        }
    ...
    private Vector getKingMoves(Coord c) {
        ...
        if (board.pieces[c.x][c.y].white) {
            if (c.x == 4 && c.y == 0 && !isAttacked(new Coord(4, 0), false)) {
                // Short
                if (board.pieces[5][0] == null && !isAttacked(new Coord(5, 0), false) && board.pieces[6][0] == null && !isAttacked(new Coord(6, 0), false) && board.pieces[7][0] != null && board.pieces[7][0].type == PieceType.ROOK && board.pieces[7][0].white) {
                    v.add(new Move(4, 0, 6, 0));
                }
                // Long
                if (board.pieces[3][0] == null && !isAttacked(new Coord(3, 0), false) && board.pieces[2][0] == null && !isAttacked(new Coord(2, 0), false) && board.pieces[1][0] == null && board.pieces[0][0] != null && board.pieces[0][0].type == PieceType.ROOK
                        && board.pieces[0][0].white) {
            ...
            if (c.x == 4 && c.y == 7 && !isAttacked(new Coord(4, 7), true)) {
                // Short
                if (board.pieces[5][7] == null && !isAttacked(new Coord(5, 7), true) && board.pieces[6][7] == null && !isAttacked(new Coord(6, 7), true) && board.pieces[7][7] != null && board.pieces[7][7].type == PieceType.ROOK && !board.pieces[7][7].white) {
                    v.add(new Move(4, 7, 6, 7));
                }
                // Long
                if (board.pieces[3][7] == null && !isAttacked(new Coord(3, 7), true) && board.pieces[2][7] == null && !isAttacked(new Coord(2, 7), true) && board.pieces[1][7] == null && board.pieces[0][7] != null && board.pieces[0][7].type == PieceType.ROOK
                        && !board.pieces[0][7].white) {
    ...
    private Vector getRealMoves(Coord c) {
        ...
        if (v != null) {
            for (int k = 0; k < v.size(); k++) {
                ...
                if (board.pieces[m.x2][m.y2] != null) {
                    v.remove(k);
                    k--;
                } else {
                    m.perform(board);
    ...
    private Vector getRealAttacks(Coord c) {
        ...
        if (v != null) {
            for (int k = 0; k < v.size(); k++) {
                Move m = (Move) v.elementAt(k);
                if (board.pieces[m.x2][m.y2] == null || board.pieces[m.x2][m.y2].white == p.white) {
                    v.remove(k);
                    k--;
                } else {
                    m.perform(board);
    ...
    public Vector getRealAll(Coord c) {
        ...
        if (v != null) {
            for (int k = 0; k < v.size(); k++) {
                ...
                if (board.pieces[mmm.x2][mmm.y2] != null && board.pieces[mmm.x2][mmm.y2].white == p.white) {
                    v.remove(k);
                    k--;
                } else {
                    mmm.perform(board);
    ...
    public boolean isAttacked(Coord c, boolean white) {
        for (byte a = 0; a < 8; a++) {
            for (byte r = 0; r < 8; r++) {
                if (board.pieces[a][r] != null && board.pieces[a][r].white == white) {
            ...
            for (byte r = 0; r < 8; r++) {
                if (board.pieces[c][r] != null) {
                    ...
                    if (v != null) {
                        for (int k = 0; k < v.size(); k++) {
                            Move m = (Move) v.elementAt(k);
                            out += board.pieces[m.x2][m.y2].getCost();
                        }
                    }
                }
            }
    ...
    public boolean checkMate() {
        return isAttacked(board.blackKing, true) && !replyForMate(false) || isAttacked(board.whiteKing, false) && !replyForMate(true);
    }
    ...
    public boolean canMove(Move m) {
        if (board.pieces[m.x1][m.y1] == null) {
            return false;
        }
    ...
    public boolean replyForMate(boolean white) {
        for (byte c = 0; c < 8; c++) {
            for (byte r = 0; r < 8; r++) {
                if (board.pieces[c][r] != null && board.pieces[c][r].white == white) {
    ...
    private Board copyBoard(Board board) {
        ...
        for (byte c = 0; c < 8; c++) {
            for (byte r = 0; r < 8; r++) {
                newBoard.pieces[c][r] = board.pieces[c][r];
            }
        }
    ...
    private void testSpeed() {
        ...
        if (stamp1 == null) {
            ...
            System.out.println("time  =  seconds: " + seconds + " milliseconds: " + millisec);
            ...
            System.out.println(empty.substring(0, col1Width - col1.length()) + col1 + empty.substring(0, col2Width - col2.length()) + col2 + empty.substring(0, col3Width - col3.length()) + col3);

            // Table values (right justified)
            System.out.println(empty.substring(0, col1Width - (dd + "").length()) + dd + empty.substring(0, col2Width - (stepcounter + "").length()) + stepcounter + empty.substring(0, col3Width - (val + "").length()) + val);
        }
    }
}
